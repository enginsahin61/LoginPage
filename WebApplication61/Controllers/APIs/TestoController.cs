...
namespace WebApplication61.Controllers.APIs
{
    ...
    public class TestoController : ControllerBase
    {
        ...
        {
            return Ok(new { Name = "Engin", Surname = "Şahin" });
        }
        ...
        public IActionResult Login([FromBody]LoginViewModel model)
        {
            if(model.Username == "aaa" && model.Password == "123123")
            {
                ...
                return Ok(new { Error = true, Message = "Token created", Data= token});
            }
            else
            {
                return BadRequest(new { Error = true, Message = "Incoreect identity."});
            }
